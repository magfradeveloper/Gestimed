  ...
  <q-page padding>
    ...
    <q-dialog v-model="mdlComp">
      <q-card style="width: 700px; max-width: 80vw">
        ...
        <q-card-section class="q-pt-none">
          ¿Desea eliminar al paciente {{ txtnomEd }} {{ txtappEd }}
          {{ txtapmEd }}?
        </q-card-section>
    ...
    <q-dialog v-model="mdlEstudio">
      <q-card style="width: 700px; max-width: 80vw">
        ...
        <q-card-section class="q-pt-none">
          <q-tabs v-model="tabEst" no-caps class="bg-grey-1 text-black">
...
export default defineComponent({
  ...
  setup() {
    ...
    function eliminarPaciente() {
      api
        .put("Paciente/desactivarPaciente", {
        ...
        .then((res) => {
          if (res.data) {
            $q.notify({
              type: "positive",
              message: "Se ha eliminado al paciente con éxito",
            });
            this.mdlComp = false;
            this.consultarPacientes();
          } else {
            $q.notify({
              type: "negative",
              message: "Ha ocurrido un error al eliminar al paciente",
            });
          }
        });
    }
        ...
        if (this.fileEstudio != null) {
          ...
          api.post("Estudios/registrarEstudio", fd, {}).then((res) => {
            if (res.data) {
              $q.notify({
                type: "positive",
                message: "Se ha registrado el estudio con éxito",
              });
    ...
    function consultarEstudios(idpac) {
      api
        .post("Estudios/consultarEstudios", {
          id_paciente: idpac,
        })
        .then((res) => {
    ...
    function verEstudio(idest) {
      api
        .post("Estudios/consultarUrlEstudio", {
          id_estudio: idest,
        })
        .then((res) => {
          if (res.data != null && res.data != "") {
            window.open(
              ruta + res.data.substring(10, res.data.length),
              "_blank"
            );
          } else {
            $q.notify({
              type: "negative",
              message: "El enlace está roto",
            });
          }
        });
    }
